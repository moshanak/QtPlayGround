	...
	, sceneGraphRenderer_(nullptr)
{
	...
	//どの QQuickWindow に属するか決まったタイミングと、QQuickWindow から外れるタイミングでも windowChanged
	//シグナルは呼ばれる
...
void SceneGraph::sync()
{
	if (!sceneGraphRenderer_)
	{
		...
		connect(window(), &QQuickWindow::beforeRendering, sceneGraphRenderer_.get(), &SceneGraphRenderer::init,
				Qt::DirectConnection);
